...
interface IconProps extends React.SVGProps<SVGSVGElement> {
  private?: boolean;
  mode?: 'Saga' | 'Rush' | string;
}

const Icon: React.FC<IconProps> = ({ private: isPrivate, mode, style, ...props }) => {
  const trophyStyle = {
    ...style,
    filter: isPrivate ? 'saturate(0.3)' : undefined
  };
    ...
    <div style={{ position: 'relative', display: 'inline-block' }}>
      ...
      {mode === 'Saga' && (
        <div style={iconOverlayStyle}>
          <FastForward style={{ width: '100%', height: '100%' }} />
        </div>
      )}
      {mode === 'Rush' && (
        <div style={iconOverlayStyle}>
          <RunningMan style={{ width: '100%', height: '100%' }} />
        </div>
      )}
    </div>
  )
