...
export default function ObsOverviewEdit(props: ObsOverviewEditProps) {
    ...
    const [mode, setMode] = useState<'bg' | 'title' | 'score' | 'active'>('bg');
    ...
    const onChangeMode = (mode: string) => {
        setMode(mode as 'bg' | 'title' | 'score' | 'active' );
        setHex(mode === 'bg' ? bg : mode === 'title' ? title : mode === 'score' ? score : active);
    }
        ...
        <div style={{display:'flex', alignItems:'baseline'}}>
          ...
          <div style={{display:'flex'}}>
            <input
                type="text"
                readOnly
                value={`${window.location.protocol}//${window.location.host}/api/obs/overview/${props.playerId}`}
                style={{ flex: '1', width: '100%', marginRight: '1rem' }}
            />
            <a href={`${window.location.protocol}//${window.location.host}/api/obs/overview/${props.playerId}`} target="_blank" rel="noreferrer">Test in browser</a>
          </div>
          <h4>Customize</h4>
            <div>
                <input
                    type="text"
                    readOnly
                    value={`${window.location.protocol}//${window.location.host}/api/obs/overview/${props.playerId}?bg=${encodeColor(bg)}&score=${encodeColor(score)}&title=${encodeColor(title)}&active=${encodeColor(active)}&max=${max}`}
                    style={{ width: '100%' }}
                />
            </div>
            <div style={{ display: 'flex', marginBottom: '1rem' }}>
                <div>
                    <fieldset>
                        <label htmlFor="max"><strong style={{ fontSize: '1.4rem' }}>{max}</strong> Show</label>
