        ...
        <div style={{display:'flex', alignItems:'baseline'}}>
          ...
          <div style={{display:'flex'}}>
            <input
                type="text"
                readOnly
                value={`${window.location.protocol}//${window.location.host}/api/obs/scores/${props.playerId}`}
                style={{ flex: '1', width: '100%', marginRight: '1rem' }}
            />
            <a href={`${window.location.protocol}//${window.location.host}/api/obs/scores/${props.playerId}`} target="_blank" rel="noreferrer">Test in browser</a>
          </div>
          <h4>Customize</h4>
            <div>
                <input
                    type="text"
                    readOnly
                    value={`${window.location.protocol}//${window.location.host}/api/obs/scores/${props.playerId}?bg=${encodeColor(bg)}&score=${encodeColor(score)}&title=${encodeColor(title)}&active=${encodeColor(active)}&max=${max}`}
                    style={{ width: '100%' }}
                />
            </div>
